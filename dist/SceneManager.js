export class SceneManager {
    ...
    createBackground(key) {
        ...
        background.setOrigin(0, 0).setDisplaySize(this.scene.cameras.main.width, this.scene.cameras.main.height);
        background.setDepth(-2);
        return background;
    }
    // Create Sand Layers
    createSandLayer(key, y) {
        const sandLayer = this.scene.add.tileSprite(0, y, this.scene.cameras.main.width, 64, // Tile height
        key);
    ...
    createImage(key, x, y, origin = [0.5, 0.5], scale = 1, depth = 0) {
    ...
    createAnimatedSeaweed(key, x, y, swayRange = 5, duration = 3000, origin = [0.5, 1], scale = 1) {
    ...
    createRockObstacle(key, x, y, scaleRange = [0.8, 1.2], duration = 1000) {
    ...
    // Create Buttons
    createButton(key, x, y, scale, callback) {
